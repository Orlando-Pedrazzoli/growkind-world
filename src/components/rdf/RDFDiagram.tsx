...
// União de literais — necessária para o next-intl tipado aceitar
// t(`elementLabels.${activeKey}`) e t(`extras.${activeKey}`)
type ElementKey = 'child' | 'environment' | 'adult';
...
export default function RDFDiagram() {
  const t = useTranslations('rdf.diagram');
  const { elements } = useRdfData();

  const [activeKey, setActiveKey] = useState<ElementKey>('child');
  const ref = useRef<HTMLElement>(null);
  const isInView = useInView(ref, { once: true, margin: '-80px' });

  const activeElement = elements.find(el => el.key === activeKey)!;
        ...
        <div className='mt-16 grid items-center gap-8 md:grid-cols-[1.2fr_1fr] lg:gap-12'>
          ...
          <AnimatePresence mode='wait'>
            ...
            >
              ...
              >
                {activeElement.desc} {t(`extras.${activeKey}`)}
              </p>
